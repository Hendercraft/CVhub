...
if($stmt = $conn->prepare($req) and session_status() === PHP_SESSION_NONE) //Si la requête retourne un résultat et qu'aucune session n'est démarrée
{
    ...
    if($stmt->num_rows == 1)
    {
        ...
        $stmt->bind_result($_SESSION['id'],$_SESSION['nom'],$_SESSION['prenom'],$_SESSION['adresse'],$_SESSION['ville'],$_SESSION['code_postal'],$_SESSION['date'],$_SESSION['email'],$_SESSION['mdp'],$_SESSION['tel'],$_SESSION['lnk'],$_SESSION['profile_pic']);
